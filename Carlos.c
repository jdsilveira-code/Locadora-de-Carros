...
void realizar_aluguel(Veiculo *vet, int *qtd, Categoria *vet_categoria, int tamanho){
    int codigoVeiculo, qtd_veiculos = *qtd;
    printf("\n--- Alugar Veiculo ---\n");
    printf("Digite o codigo do veiculo que deseja alugar: ");
    scanf(" %d", &codigoVeiculo);
    for(int i = 0; i < qtd; i++){
        if(vetor_veiculo[i].codigo == codigoVeiculo){
            break;
        }
    }
    if(i == qtd_veiculos){
    ...
    do{
        ...
        case 6:
            if(qtd_veiculos <= 0){
                ...
                realizar_aluguel(&vetor_veiculo[qtd_veiculos], &qtd_veiculos, vetor_categoria, qtd_cadastrados));
            }
